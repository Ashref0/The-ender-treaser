...
{
	m->map=zoomSurface(bg,0.1,0.1,1);// échelle 1/10
	m->pos_map.x = 970;// equiv (*m).pos_map.x=1000;
	m->pos_map.y = 100;
...
{
	m->perso_pos_map.x=p->x/10+m->pos_map.x;
	m->perso_pos_map.y=p->y/10+10+m->pos_map.y;// besh l position ta" l mini perso iji kad kad maa l perso lekbir 3al axe y 
}
...
//erreurs de compilation dans ces deux fonctions
/*
